            ...
            switch (typeof query_value) {
                ...
                case 'string':
                    ...
                    while ((groups = regex.exec(query_value)) !== null) {
                        ...
                        if (groups.length !== 3) {
                            error = callback(new Errors.QueryError('wrong syntax:', groups[0] || 'no query'));
                            break;
                        }
                    ...
                    if (query_value.length !== processed_query.length)
                        error = new Errors.QueryError('wrong syntax: evaluated ' + processed_query, ' vs. ', query_value);

                    key_res.is_number = true;
                    key_res.expression = query_value;
                    key_res.match = evalNumberExpression.bind(this, expressions); //TODO regex wie bei string hier bilden?
                    break;
                default:
                    error = new Errors.QueryError(`query for "${query_key}" must be a number or an expression string, but was: ${typeof query_value}`);
                    break;
