...
module.exports = function (query, resolved_schema, callback) {
    //TODO: resolved_schema must always return true for typeOf number, if it is a number!
    ...
    for (let key of keys) {
        ...
        if (resolved_schema.value(key).typeOf('string')) {
            ...
            const evalExpression = (qvalue) => {
                let found = '', groups;

                console.log(qvalue);

                while ((groups = regex.exec(qvalue)) !== null) {
                    let op, val;
                    ...
                    // The result can be
                    // ;accessed through the `groups`-variable.
                    groups.forEach((match, i) => {
                        ...
                        console.log('left:', qvalue.length, found.length);
                        console.log(`Found match, group ${i}: ${match}`);
                        console.log('eval:', eval(3 + op + val));
                    });
                }
            };

            evalExpression(' < 1.0  >  2.0 ');
            ...
            switch (typeof query_value) {
                ...
                default:
                    ...
                    error = new Error(`Query for "${key}" must be a number or a string but was: ${typeof query_value}`); //TODO: !return will not work here
                    break;
