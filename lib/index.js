...
const evalExpression = (query_value, value, callback) => {
    ...
    while ((groups = regex.exec(query_value)) !== null) {
        ...
        if (groups.length !== 3)
            return callback(new Error('query synthax seems not to be right:', groups[0] || 'no query'));
        ...
        console.log('eval:', value, found, expr_res);
        if (!expr_res)
            return callback(null, false);
    }

    if (query_value.length !== found.length)
        return callback(new Error('query synthax not right: ' + found, query_value));
...
module.exports = function (query, resolved_schema, callback) {
    ...
    for (let key of keys) {
        ...
        if (resolved_schema.value(key).typeOf('string')) {
            ...
            evalExpression(' < 1.0  >  2.0 ', 3, (err, res) => {
                console.log(err, res);
            });
