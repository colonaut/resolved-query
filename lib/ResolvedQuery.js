...
const construct = (items, callback) => {
    ...
    if (Privates.get(this).query_result)
    ...
    Object.assign(Privates.get(this), {query_result: query_result}; //TODO: deal with checking if query_result exists
...
class ResolvedQuery {
     ...
     */
    isNumber(key) {
        return construct((err, query_data) => !!query_data[key].is_number);
     ...
     */
    isArray(key) {
        return !!query_data[key].is_array;
    }
     ...
     */
    expr(key) {
        return query_data[key].expression
    };
