    ...
    if (Privates.get(this).resolved_schema.value(query_key).typeOf('string')) {
        ...
        switch (typeof query_value) {
            ...
            case 'string':
                ...
                break;
            default:
                return callback(new Errors.QueryError(`Type mismatch: query for "${query_key}" must be a number or an expression string, but was: ${typeof query_value}`));
                break;
        ...
        if (!Array.isArray(query_value))
            return callback(new Errors.QueryError(`Query for "${query_key}" must be an array but was: ${typeof query_value}`));
...
function matchModel(query_keys, model, callback) {
    let query_data = Privates.get(this).query_result;
    let query_key = query_keys.shift();

    if (!model[query_key])
        return callback(new Errors.QueryError(`Key mismatch: model does not contain key "${query_key}"`), false);

    if (typeof query_data[query_key].match !== 'function')
        return callback(new Errors.QueryError(`Query could not find match function for key "${query_key}"`), false);
    ...
    query_data[query_key].match(model[query_key], (err, res) => {
        ...
        matchModel.bind(this)(query_keys, model, callback);
    });
}
...
class ResolvedQuery {
     ...
     * @returns {Promise} if no callback is given//TODO: should this return the model or null instead of bool? bool for non callback, model for with callback?
     */
    match(model, callback) {
        if (typeof callback === 'function'){
            resolveQuery.bind(this)((err) => {
                if (err)
                    return callback(err, false);

                return matchModel.bind(this)([].concat(Privates.get(this).query_keys), model, callback);
            });
        }
        ...
        return new Promise((resolve, reject) => {
            resolveQuery.bind(this)((err) => {
               if (err)
                ...
                matchModel.bind(this)([].concat(Privates.get(this).query_keys), model, (err, res) => {
                    if (err)
                        return reject(err);
